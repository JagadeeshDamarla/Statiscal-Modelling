...
- `store_id` / `store_location` – Location of the coffee shop  
- `product_id` – Unique product identifier  
- `product_category` / `product_type` / `product_detail` – Product information  
- `transaction_qty` – Quantity of product sold  
- `unit_price` – Price per unit  

**Derived Features (from analysis):**
- `revenue` = `transaction_qty * unit_price`  
- `hour_of_day`, `day_of_week`, `month` – derived from `transaction_date` and `transaction_time`  
...
- The dataset spans **Jan 2023 – Jun 2023**, with no missing dates in the records.  
